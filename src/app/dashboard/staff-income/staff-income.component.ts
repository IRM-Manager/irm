...
import { payee1, payee2 } from '../shared/form';
...
export class StaffIncomeComponent implements OnDestroy, OnInit {
  ...
  GetPayeeDataEvent() {
    ...
    switch (this.selected_year?.is_file) {
      case 'file':
        console.log('file');
        this.viewMode = 'file';
        this.feedbackForm.controls['year'].setValue(
          `${this.selected_year?.year?.id}|${this.selected_year?.year?.year}`
        );
        break;
      case 'input':
        console.log('input');
  ...
  onSubmit() {
    ...
    // "pension":true,
    const datas = {
      employeeTin: this.feedback1.tin,
      basic: this.feedback2.basic,
      yearId: get_year[0],
      housing: this.feedback2.housing,
      pension: false,
      tp: this.feedback2.transport,
      nhis: this.feedback2.other || parseFloat('0.0'),
      employee_position: this.feedback1.position,
    };
    console.log('form daata', datas);
    this.httpService.postData(datas, this.data2?.payer?.tin).subscribe(
      // get_year[0]
      (data: any) => {
        this.loading = false;
        this.disabled = false;
  ...
  onUpdate() {
    ...
    console.log('form daata', datas);
    this.httpService
      .updateData(datas, this.data2?.payer?.tin, get_year[0])
      .subscribe(
        (data: any) => {
          this.loading = false;
          this.disabled = false;
      ...
      .subscribe(
        ...
        (err) => {
          ...
          if (err.status === 500) {
            this.snackBar.open(
              err.error?.detail ||
                'Invalid data format or File not Valid! (Should be CSV)',
              '',
              {
                duration: 5000,
                panelClass: 'error',
            ...
            this.snackBar.open(
              err.error?.status || 'Error Uploading File',
              '',
              {
                duration: 5000,
                panelClass: 'error',
  ...
  remove(id: any, index2: number) {
    ...
    if (check[0].employeeTin == form_data.tin) {
      ...
      this.httpService
        .deleteData(BaseUrl.delete_update_payer, id + '/')
        .subscribe(
          (data: any) => {
